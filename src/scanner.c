...
static Token
make_token(TokenType token_type)
{
  Token token;
  ...
  switch (scanner.start[0])
  {
    ...
    case 'f':
      if (scanner.current - scanner.start > 1)
      {
        switch (scanner.start[1])
        {
          ...
          case 'o': return check_keyword(2, 1, "r", TOKEN_OR);
    ...
    case 's':
      if (scanner.current - scanner.start > 1)
      {
        switch (scanner.start[1])
        {
          case 's': return check_keyword(2, 4, "uper", TOKEN_SUPER);
          case 'e': return check_keyword(2, 2, "lf", TOKEN_THIS);
        }
      }
    break;
